...
class employee{
    private:
        ...
        void listEmployees(void){ //To list total employees with Name, Id and Designation
            ...
            cout<<"\n NAME        |     ID     |     DESIGNATION\n";
            cout<<"----------------------------------------------";
            while(fscanf(file, "%s %s %s %d %d %d %f", &name[0], &id[0] , &designation[0], &age, &ctc, &experience, &salary)!= EOF)
                cout<<"\n"<<name<<"\t\t"<<id<<"\t\t"<<designation;
            fclose(file);
            waitForEnter();
        }
        ...
        void editExisting(void){ //edits Designation  CTC Age Experience and salary of an employee
            system("cls");
            char checkId[5];
            cout<<"\nEnter employee id: ";
            cin>>checkId;
            ...
            while(fscanf(file, "%s %s %s %d %d %d %f", &name[0], &id[0] , &designation[0], &age, &ctc, &experience, &salary)!=EOF){
                if(strcmp(checkId, id)==0)
                    fprintf(tempfile, "%s %s %s %d %d %d \n", name, id, newDesignation, newage, newCtc, newExperience, newSalary );
                else
                    fprintf(tempfile, "%s %s %s %d %d %d \n", name, id, designation, age, ctc, experience, salary );
            }
            fclose(file);
            fclose(tempfile);
        ...
        void deleteEmployeeDetails(void){ //removing records
            ...
            if(ch=='y'){
                ...
                file= fopen("data.txt", "r");
                tempfile= fopen("temp.txt", "w");
                while(fscanf(file, "%s %s %s %d %d %d", &name[0], &id[0] , &designation[0], &age, &ctc, &experience, &salary)!=EOF)
                    if(strcmp(checkId, id)!=0)
                        fprintf(tempfile, "%s %s %s %d %d %d %f\n", name, id, designation, age, ctc, experience, salary );
                fclose(file);
                fclose(tempfile);
                int isRemoved= remove("data.txt");
                int isRenamed= rename("temp.txt", "data.txt");
